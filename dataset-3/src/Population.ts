import { testData, trainData } from ".";
import { calculateFitness } from "./Algorithm";
import { Individual } from "./Individual";

export interface PopulationFitness {
  best: number;
  average: number;
  worst: number;
}
export class Population {
  ...
  calculateFitness(): PopulationFitness {
    let best = 0;
    let average = 0;
    let worst = calculateFitness(testData, this.population[0]);
    let totalFitness = 0;

    for (let individual of this.population) {
      const currentFitness = calculateFitness(testData, individual);
